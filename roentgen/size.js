...
function sizeRun() {
    ...
    if (sizeIterator == 0 && sizeModeIndex == 0) {
        ...
        sizeIconIndex =
            sizeIconIndex < sizeIcons.length - 1 ? sizeIconIndex + 1 : 0;
        sizeIconFill.setAttribute(
            "transform",
            "translate(" + toCoordinates(sizeCenter) + ") "
    ...
    if (sizeMode.action == "change") {
        ...
        p = [
            [[-s - tipLength, -s], [s + tipLength, -s]],
            [[-s - tipLength, s], [s + tipLength, s]],
            [[-s, -s - tipLength], [-s, s + tipLength]],
            [[s, -s - tipLength], [s, s + tipLength]],
        ]
        for (i in [...Array(4).keys()]) {
        ...
        sizeText.setAttribute("x", sizeCenter[0] + s + 70)
